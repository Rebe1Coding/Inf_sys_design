...
class ConsoleUI:
    ...
    def use_json_repository(self):
        ...
        filename = input("📝 Введите имя файла JSON (по умолчанию: clients.json): ").strip()
        if not filename:
    ...
    def use_yaml_repository(self):
        ...
        filename = input("📝 Введите имя файла YAML (по умолчанию: clients.yaml): ").strip()
        if not filename:
    ...
    def show_repo_info(self):
        ...
        if self.current_repo:
            ...
            print(f"   Время подключения: {self.history[-1]['time'] if self.history else 'N/A'}")
        else:
    ...
    def run(self):
        ...
        while True:
            ...
            print("\n" + "="*50)
            continue_choice = input("\n🔄 Нажмите Enter для выбора нового репозитория или 'q' для выхода: ").strip().lower()
