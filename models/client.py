...
class Client:
    ...
    def validate_name(name: str) -> str:
        if not re.match(r"^[А-ЯЁ][а-яёА-ЯЁ\s\.\-]+$", name):
            raise ValueError(
                "Название/ФИО должно начинаться с заглавной буквы и содержать только буквы, пробелы, тире или точки"
            )
        return name.strip()
    ...
    def validate_phone(phone: str) -> str:
        ...
        if not re.match(r"^\+?\d{7,15}$", cleaned_phone):
            raise ValueError(
                "Телефон должен содержать от 7 до 15 цифр и может начинаться с +"
            )
